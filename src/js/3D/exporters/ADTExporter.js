				...
				const exportObjects = async (exportType, objects, csvName) => {
					...
					for (const model of objects) {
						...
						try {
							...
							csv.addRow({
								ModelFile: modelFile,
								PositionX: model.Position?.[0] ?? model.position?.[0] ?? model.Pos?.[0] ?? 0,
								PositionY: model.Position?.[1] ?? model.position?.[1] ?? model.Pos?.[1] ?? 0,
								PositionZ: model.Position?.[2] ?? model.position?.[2] ?? model.Pos?.[2] ?? 0,
								RotationX: model.Rotation?.[0] ?? model.rotation?.[0] ?? model.Rot?.[0] ?? 0,
								RotationY: model.Rotation?.[1] ?? model.rotation?.[1] ?? model.Rot?.[1] ?? 0,
								RotationZ: model.Rotation?.[2] ?? model.rotation?.[2] ?? model.Rot?.[2] ?? 0,
								RotationW: model.Rotation?.[3] ?? model.rotation?.[3] ?? model.Rot?.[3] ?? 1,
								ScaleFactor: model.scale !== undefined ? model.scale / 1024 : 1,
								ModelId: model.uniqueId ?? model.ID ?? 0,
								Type: csvName,
